...
exports.sendModelIsRunningEmail = exports.alertBilling = exports.addUserToContacts = exports.sendFeedbackEmails = void 0;
const functions = require("firebase-functions");
const firebase_1 = require("./firebase");
exports.sendFeedbackEmails = functions.firestore.document('/feedback/{feedbackID}').onCreate(async (change, context) => {
...
exports.addUserToContacts = functions.firestore.document('users/{uid}').onCreate(async (change, context) => {
...
exports.alertBilling = functions.firestore.document('users/{uid}').onUpdate(async (change, context) => {
...
exports.sendModelIsRunningEmail = functions.firestore.document('projects/{projectId}/models/{modelId}').onUpdate(async (change, context) => {
    ...
    if (oldModelData.lifecycleStatus && newModelData.lifecycleStatus) {
        if (oldModelData.lifecycleStatus.running === false && newModelData.lifecycleStatus.running) {
            const query = await (0, firebase_1.getDoc)(`/users/${context.params.projectId}`);
            if (query.data()) {
