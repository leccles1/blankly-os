...
exports.reportUsage = functions.pubsub
    .schedule('0 */6 * * *')
    .onRun(async () => {
    await runUsage();
});
...
async function getClientCreditUsage(usageApiClient, options) {
    return await usageApiClient.get('/usage', {
        params: options
        // @ts-ignore
    }).then(res => res.data.credits) ?? 0;
}
async function checkStripelessUsage(client, usageApiClient) {
...
async function runUsageForClient(client, usageApiClient) {
...
async function notifyCreditsOwed(client, credits) {
    // const templateData = {
